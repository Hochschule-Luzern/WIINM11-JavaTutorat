package ch.hslu.wi.block09b_Uebung01_Groesse_Interface_solution;

import java.util.LinkedList;
import java.util.List;

public class Groesse_Main_Ueb01 {
	...
	public static void main(String[] args) {
		...
		// Berechnung der Gesamtfläche der in der Liste gespeicherten "Grössen"-Objekten
		double summe = 0;
		for (int i = 0; i < l2.size(); i++) {  /* Groesse it: l2   */
			summe = summe + grundflaeche(l2.get(i));
		}

		//Kürzer mit neuer "foreach" - Art:
		double summe1 = 0;
		for (Groesse it: l2){
			summe1 = summe1 + grundflaeche(it);
		}
		summe1 = summe1 - grundflaeche(f1);
		
		System.out.println("Die Fläche beträgt: " + summe1 + " (ohne Fussballplatz)");
		System.out.println("Die Gesamtfläche beträgt: " + summe+ " (mit Fussballplatz)");
	}
}
